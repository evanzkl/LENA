...
static esp_err_t upload_job_handler(httpd_req_t *req) {
  char *buf = NULL;
  char job_id[64] = {0};
  ...
  bool has_backend = have_query && httpd_query_key_value(buf, "backend_url", backend_url_raw, sizeof(backend_url_raw)) == ESP_OK;
  if (buf) {
    free(buf);
  }
...
void startCameraServer() {
  ...
  config.stack_size = 8192;  // HTTPClient's outbound POST needs more stack than the default.
  ...
  httpd_uri_t upload_job_uri = {
    .uri = "/upload_job",
    .method = HTTP_GET,
    .handler = upload_job_handler,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
